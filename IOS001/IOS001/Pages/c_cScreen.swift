    ...
    var body: some View {
        ScrollView{
            HStack{
                ...
                Spacer()
                Text("您的有條件釋放令將會持續多久？").lineLimit(3).font(Font.system(size: 25)).fontWeight(.bold).frame(maxWidth: .infinity, alignment: .center).foregroundColor(.white).onTapGesture {
                    tapped = !tapped
                }
                Spacer()
            ...
            if tapped {
                ...
                }.background(Color.gray).opacity(0.7).padding(20).onTapGesture {
                    tapped = false
                }
            }
