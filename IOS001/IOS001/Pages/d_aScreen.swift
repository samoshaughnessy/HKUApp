        ...
        ScrollView{
            ...
            VStack(alignment:.leading){
                ...
                HStack{
                    ...
                    Spacer()
                    Text("如果有條件釋放令不合理 ▼").lineLimit(4).font(Font.system(size: 25)).fontWeight(.bold).frame(maxWidth: .infinity).multilineTextAlignment(.center).foregroundColor(.black).onTapGesture {
                        tapped = !tapped
                    }
                    Spacer()
                }
                ...
                Text("\n如果有條件釋放令不合理").lineLimit(4).font(Font.system(size: 30)).fontWeight(.bold).frame(maxWidth: .infinity).multilineTextAlignment(.center
                )
